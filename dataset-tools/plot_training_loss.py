...
# Plot training loss for each model with different colors
plt.plot(df_unet['Epoch'], df_unet['Train_Loss'], label='UNet', color='blue')
plt.plot(df_fpn['Epoch'], df_fpn['Train_Loss'], label='FPN', color='green')
plt.plot(df_pspnet['Epoch'], df_pspnet['Train_Loss'], label='PSPNet', color='red')
plt.plot(df_deeplab['Epoch'], df_deeplab['Train_Loss'], label='DeepLabV3+', color='purple')

# Set plot labels and title with LaTeX formatting
plt.xlabel(r'\textbf{Epochs}')
plt.ylabel(r'\textbf{Training Loss}')
#plt.title(r'\textbf{Training Loss Over Epochs for Different Models}')
...
plt.savefig('training_loss_plot.png', dpi=300, bbox_inches='tight')  # PNG format
# plt.savefig('training_loss_plot.pdf', bbox_inches='tight')  # PDF format
